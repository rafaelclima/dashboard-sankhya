...
import { arquivoConciliacaoKabum } from '@/types/kabum.types'
import { format } from 'date-fns'
import { ptBR } from 'date-fns/locale'

interface CardPlacesProps {
  data: arquivoConciliacaoKabum[]
}
...
export function CardInfoPorArquivo({ data }: CardPlacesProps) {
  return (
    <>
      {data.map((item) => (
        <div
          ...
          <ItemCard
            ...
            value={format(new Date(item.DH_REPASSE), 'dd/MM/yyyy', { locale: ptBR })}
          ...
          <ItemCard
            title="Débito"
            value={formatMoeda(item.DEBITO)}
            className={getTextColor(item.DEBITO)}
            icon={<FaSackDollar size={32} className="m-auto w-8 h-8" />}
          />
          <ItemCard
            title="Crédito"
            value={formatMoeda(item.CREDITO)}
            className={getTextColor(item.CREDITO)}
            icon={<FaMoneyBillTrendUp size={32} className="m-auto w-8 h-8" />}
          />
          <ItemCard
            title="Saldo"
            value={formatMoeda(item.SALDO)}
            className={getTextColor(item.SALDO)}
